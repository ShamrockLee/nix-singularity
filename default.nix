{ src ? ./srcs/wip.nix, nixpkgs ? <nixpkgs>, system ? builtins.currentSystem }:

let
  inherit (pkgs) singularity-tools stdenv buildEnv writeText;
  ...
  content = stdenv.mkDerivation {
    ...
    installPhase = ''
      ...
      # ln -s ${stdenv.shell} $out/bin/sh # Will be linked inside buildImage
  ...
  image = singularity-tools.buildImage rec {
    name = "nix-base";
    ...
    environment = ''
      ...
      export NIX_PATH=nixpkgs=${unstable}
  ...
  baseSingularity = writeText "Singularity" ''
...

{
  ...
  inherit image content path unstable;
}
