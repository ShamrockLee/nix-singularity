...
rec {
  ...
  buildImage = {
    ...
    let layer = mkLayer {
        ...
        environmentFile = shellScript "environment.sh" environment;
        runScriptFile = shellScript "run-script.sh" runScript;
        result = vmTools.runInLinuxVM (
          runCommand "singularity-image-${name}.sif" {
          ...
          ''
            ...
            ${lib.optionalString (environment != null) ''
              ln -s ${environmentFile} .singularity.d/env/90-environment.sh
            ''}
